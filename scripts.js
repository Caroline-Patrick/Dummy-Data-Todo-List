    ...
    const populateTodos = (d) => { //defining function populateTodos and inputting the argument of the JSON-styled data, 'd'
       
       
        const todoList = document.getElementById("todo-list") //the ordered list (ol) element becomes the variable - 'todoList'
        ...
        for (let i = 0; i < d.data.length; i++) {
            ...
            element.innerHTML = `<img src="${d.data[i].avatar}">` // these are directions to go into the created 'li' element (variable name =element) and add to it some HTML code. Specifically, we are adding it as an image. The image we are adding is the 'avatar' key from each JSON object. 
